...
### ❌ Problem 6 → ✅ Resolved: Class Imbalance

**Before:** Naive models predict "No Churn" for everyone → 0% recall on churners.

**After:** SMOTE oversampling + `class_weight="balanced"` + PR-AUC as the optimisation target:

```
Before SMOTE:   Churn = 26%,  No Churn = 74%  →  Recall:  31%
After SMOTE:    Churn = 50%,  No Churn = 50%  →  Recall:  75.8%

Improvement:  +44.8 percentage points in recall
```
...
```
══════════════════════════════════════════════════════════════════════
  CUSTOMER CHURN PREDICTION — ADVANCED ML PIPELINE
  Started at: 2025-01-15 10:30:00
══════════════════════════════════════════════════════════════════════

[STEP 2]  Dataset shape: (10000, 21) | Churn rate: 26.41%
[STEP 4]  Features after engineering: 35
[STEP 7]  After SMOTE — Train: 13,706 rows
[STEP 8]  XGBoost        CV AUC = 0.9231 ± 0.0038
          LightGBM       CV AUC = 0.9267 ± 0.0035
[STEP 9]  Best RF params: {n_estimators: 350, max_depth: 11}
          Best CV AUC: 0.9210
[STEP 10] Fitting stacking ensemble...
          ✓ Stacking ensemble trained.
[STEP 11] ★ Stacking Ensemble — ROC-AUC: 0.9421 | F1: 0.8283
[STEP 13] Saved to model_artefacts/
══════════════════════════════════════════════════════════════════════
  PIPELINE COMPLETE
══════════════════════════════════════════════════════════════════════
```
